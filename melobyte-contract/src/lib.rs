...
impl melobyte {
    pub fn initialize(env: Env, admin: Address, asset: Address, price: i128) {
        ...
        melobyteDataKey::TokenId
            .bump(&env, MAX_BUMP)
            .set::<u32>(&env, &0);
        melobyteDataKey::AssetAddress
            .bump(&env, MAX_BUMP)
            .set::<Address>(&env, &asset);
        melobyteDataKey::Price
            .bump(&env, MAX_BUMP)
            .set::<i128>(&env, &price);
...
impl melobyte {
    ...
    pub fn mint(env: Env, x: u32, y: u32, to: Address) -> Result<u32, Error> {
        ...
        // Pay the NFT
        let asset = melobyteDataKey::AssetAddress.get::<Address>(&env).unwrap();
        let price = melobyteDataKey::Price.get::<i128>(&env).unwrap();
        token::Client::new(&env, &asset).transfer(&to, &erc721::get_admin(&env), &price);
        // Retrieve the token id to mint
        let token_id: u32 = melobyteDataKey::TokenId.get(&env).unwrap_or(0);
        // Check if we reached the max supply
        if token_id > MAX_SUPPLY {
            //return Err(melobyteError::Exhausted);
            panic!("Exhausted")
        }
        // Compute and store the next token id
        melobyteDataKey::TokenId.set::<u32>(&env, &(token_id + 1));
        Coords::Token(x, y)
            .bump(&env, MAX_BUMP)
            .set(&env, &token_id);
    ...
    pub fn token_uri(env: Env, token_id: u32) -> String {
        if token_id < melobyteDataKey::TokenId.get(&env).unwrap_or(0) {
