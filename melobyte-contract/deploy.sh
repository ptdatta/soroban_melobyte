WASM_HASH=$(soroban contract install --wasm ./target/melobyte-prod.wasm --source admin --network standalone)
echo $WASM_HASH
CONTRACT_ID=$(soroban contract deploy --wasm ./target/melobyte-init.wasm --source --network)
echo $CONTRACT_ID
